...
import { OrbitControls, Center, Html, Bounds, Edges } from"@react-three/drei";
...
function Model() {
 ...
 React.useEffect(() => {
 loader.load("/Unicorn.stl", (geometry) => {
 geometry.computeVertexNormals();
 geometry.center();
 setGeom(geometry);
 }, undefined, (err) => {
 console.log("Unicorn model not found or invalid", err);
 });
 }, []);
 ...
 <meshStandardMaterial color="#ffffff" roughness={0.15} metalness={0.4} emissive="#000000"
 />
 <Edges threshold={25} color="#000000" opacity={0.1} transparent />
 </mesh>
 );
}
...
export function SpecialProducts() {
 ...
 <span className="text-bee-yellow text-[9px] md:text-[10px] font-bold uppercase tracking-[0.2em] md:tracking-[0.4em] block mb-2 md:mb-4">3D-Студия_BBee</span>
 ...
 <Canvas dpr={[1, 1.5]} gl={{ antialias: true, powerPreference:"high-performance", alpha: true }} camera={{ position: [500, 500, 500], fov: 40 }}
 >
 ...
 <OrbitControls autoRotate autoRotateSpeed={1.5} enableZoom={false} enableRotate={true}
 enablePan={false}
 />
