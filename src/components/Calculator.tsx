...
function Model({ url, onSpecsUpdate, manualScale, autoRotate = true, isCtrlPressed = false }: { url: string, onSpecsUpdate: (specs: any) => void, manualScale: number, autoRotate?: boolean, isCtrlPressed?: boolean }) {
 ...
 geometry.center();
 const vol = calculateVolume(geometry); const area = calculateSurfaceArea(geometry);
 const box = geometry.boundingBox;
 const size = new THREE.Vector3();
 if (box) {
 box.getSize(size);
 // If the model is clearly Z-up (tall along Z but short along Y), // rotate it 90 degrees around X to make it Y-up for Three.js
 ...
 onSpecsUpdate({
 volume: baseSpecs.current.volume, surfaceArea: baseSpecs.current.surfaceArea,
 baseDimensions: { x: baseSpecs.current.size.x, y: baseSpecs.current.size.y, z: baseSpecs.current.size.z },
 dimensions: { x: baseSpecs.current.size.x * manualScale, y: baseSpecs.current.size.y * manualScale, z: baseSpecs.current.size.z * manualScale },
 ...
 <meshStandardMaterial color="#ffffff" roughness={0.15} /* Sharper reflections for detail */
...
function Loader() {
 ...
 <motion.div animate={{ x: [-128, 128] }} transition={{ duration: 1.2, repeat: Infinity, ease:"linear" }}
...
function ModelPreview({ url, onSpecsUpdate, manualScale = 1, isFullScreen = false, controlMode = 'orbit', isCtrlPressed = false }: { url: string, onSpecsUpdate: (specs: any) => void, manualScale?: number,
 ...
 <Canvas shadows camera={{ position: [1000, 1000, 1000], fov: 35, far: 100000 }} gl={{ antialias: true, alpha: true, powerPreference: "high-performance" }} performance={{ min: 0.5 }}>
 <color attach="background" args={["#060606"]} />
 <ambientLight intensity={0.7} />
 <hemisphereLight intensity={1.5} color="#ffffff" groundColor="#111111" />
 <directionalLight position={[3000, 5000, 8000]} intensity={3.5} castShadow shadow-mapSize={[1024, 1024]} shadow-bias={-0.0001}
 />
 ...
 <ContactShadows position={[0, -20, 0]} opacity={0.8} scale={15000} blur={2.5} far={800} frames={1} resolution={256} />
 <OrbitControls makeDefault enabled={!isCtrlPressed}
 autoRotate={false} maxDistance={40000}
 minDistance={10}
 enableDamping
 dampingFactor={0.05}
...
export function Calculator() {
 ...
 // If it's the default object {name:"Unicorn.stl"}, we don't change fileUrl
 }, [file]);
 ...
 <button onClick={() => setControlMode(controlMode === 'orbit' ? 'pan' : 'orbit')}
 ...
 <h3 className="text-xl md:text-2xl font-bold uppercase tracking-tight mb-2 md:mb-4">Загрузите 3D-модель</h3>
 ...
 <span className="text-[10px] md:text-xs font-mono text-bee-white tracking-tight truncate">{file.name}</span>
 </div>
 ...
 <span className="text-bee-yellow text-[9px] md:text-[10px] font-bold uppercase tracking-[0.2em] md:tracking-[0.4em] block mb-2 md:mb-4">Калькулятор</span>
 <h2 className="text-4xl md:text-5xl lg:text-5xl font-black uppercase tracking-tight leading-tight mb-6 md:mb-8">
 Рассчитать<br /><span className="text-bee-yellow">стоимость</span>
 </h2>
 <p className="text-xs md:text-sm text-bee-text-muted font-light leading-relaxed max-w-md">
 *Если у вас есть своя 3D модель для загрузки и дальнейшей печати, пожалуйста, предоставьте её нам.
 ...
 {specs && (
 <span className="text-[8px] sm:text-[9px] uppercase text-bee-yellow/50 font-mono tracking-tight">
 {isSolid ? '*100% заполнено (Монолит)' : `*Полая оболочка ${wallThickness}мм + 1% заполнения`}
 </span>
 )}
 </div>
 <div className="flex items-center gap-2 self-end sm:self-auto">
 <span className="text-xl sm:text-2xl md:text-3xl font-black text-bee-yellow tracking-tight whitespace-nowrap">
 ...
 {isSolid && (
 <p className="text-[8px] uppercase text-red-500/70 font-bold tracking-tight animate-pulse">
 *Внимание: значительно увеличивает вес и стоимость.
 ...
 <input type="number" value={Math.round(specs.dimensions[axis])}
 ...
 <p className="text-[10px] text-bee-white/20 uppercase tracking-widest relative z-10">Цена может измениться после проверки инженером</p>
 <AnimatePresence>
 {!file && (
 <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-x-0 bottom-0 top-0 bg-bee-gray/90 backdrop-blur-sm z-20 flex items-center justify-center px-8 text-center transition-all"
 >
 ...
 <input type="text" placeholder="@username или +7..."
 ...
 className="flex-grow py-4 bg-bee-yellow text-bee-black text-[10px] font-bold uppercase tracking-[0.3em] hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-3 shadow-[0_10px_20px_rgba(255,215,0,0.1)] [.light_&]:shadow-none"
 >
 ...
 <h3 className="text-3xl font-black uppercase tracking-tight">Заявка принята</h3>
