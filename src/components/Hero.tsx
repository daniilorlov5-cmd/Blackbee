...
function BeeLogoModel() {
  ...
  const wingShape = useMemo(() => {
    ...
    // Top point (the vertex where wings meet)
    shape.moveTo(0, 1.2);
    // Outer bottom sharp point
    shape.lineTo(-0.9, -1.2);
    // Inner bottom point (creating the cutout)
    ...
    <section className="relative flex pt-24 md:pt-32 pb-16 md:pb-24 lg:pl-12 bg-bee-black">
      ...
      <div className="flex-grow flex items-center relative z-10 px-6 md:px-12 lg:px-20 overflow-hidden">
          ...
          >
            <div className="w-full h-full max-w-[1000px] max-h-[1000px]">
              <Canvas camera={{ position: [0, 0, 8], fov: 35 }} dpr={[1, 2]} gl={{ alpha: true }}>
                ...
                <OrbitControls enableZoom={true} makeDefault rotateSpeed={0.5} />
              </Canvas>
            </div>
          </motion.div>
        </div>
        ...
        <div className="relative w-full max-w-7xl mx-auto h-full flex items-center z-20 mt-10 md:mt-0 pointer-events-none">
          ...
          >
            <div className="mb-6">
              <span className="text-bee-yellow text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] md:tracking-[0.4em] bg-bee-yellow/5 px-4 py-2 border border-bee-yellow/20 inline-block mb-4">Аддитивное производство полного цикла</span>
            </div>
            ...
            <h1 className="text-[14vw] md:text-[7vw] lg:text-[90px] leading-[0.9] font-black tracking-tighter uppercase mb-6 md:mb-10 italic skew-heading drop-shadow-2xl max-w-2xl">
              Идея —<br className="hidden md:block" /> в <span className="text-bee-yellow">объект</span><br />за 48 часов
            </h1>
            ...
            <div className="flex flex-wrap gap-4 mb-12 md:mb-20">
              <button 
                onClick={() => openModal()}
                className="w-full sm:w-auto px-8 md:px-12 py-4 md:py-5 lg:px-14 lg:py-6 bg-bee-yellow text-bee-black text-[11px] md:text-[13px] font-black uppercase tracking-[0.2em] hover:scale-105 transition-all shadow-[0_0_40px_rgba(255,215,0,0.2)]"
              >
                Узнать стоимость
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 lg:gap-8 border-t border-bee-border pt-8 md:pt-10">
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] text-bee-text-muted uppercase font-bold tracking-[0.2em] opacity-40">Скорость</span>
                </div>
                <span className="text-xs font-mono text-bee-white font-bold tracking-widest leading-none uppercase">От 48 часов</span>
              </div>
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  <Cpu className="w-3.5 h-3.5 text-bee-yellow" />
                  <span className="text-[9px] text-bee-text-muted uppercase font-bold tracking-[0.2em] opacity-40">Логистика</span>
                </div>
                <span className="text-xs font-mono text-bee-yellow font-bold tracking-widest leading-none uppercase">По всей РФ</span>
              </div>
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  <ShieldCheck className="w-3.5 h-3.5 text-bee-yellow" />
                  <span className="text-[9px] text-bee-text-muted uppercase font-bold tracking-[0.2em] opacity-40">Точность</span>
                </div>
                <span className="text-xs font-mono text-bee-white font-bold tracking-widest leading-none uppercase">До 0.01 мм</span>
              </div>
            </div>
          </motion.div>
        </div>
