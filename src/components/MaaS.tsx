...
export function MaaS() {
 ...
 <h2 className="text-5xl md:text-5xl lg:text-[64px] xl:text-[72px] font-black uppercase tracking-tight leading-tight mb-8 md:mb-10 text-bee-white drop-shadow-lg">
 <span className="inline-block">ВАШ СКЛАД</span>
 <br />
 <span className="text-bee-yellow inline-block mt-2 md:mt-3">В ОБЛАКЕ</span>
 </h2>
 <p className="text-base md:text-lg text-bee-text-muted max-w-md font-light">
 Забудьте о MOQ и переполненных складах. Black Bee — это «Zero Stock» производство. Печатаем тысячи единиц продукции — от подставок для книг до корпоративного мерча. В нашей базе более 6000 готовых к запуску SKU. Основной фокус — скорость, масштабируемость и автоматизация.
 </p>
 </div>
 ...
 <h4 className="text-sm md:text-base font-bold uppercase tracking-[0.2em] mb-6 md:mb-8 text-bee-yellow pt-2 sm:pt-0">Экономика Black Bee</h4>
 ...
 <span className="text-lg md:text-xl font-bold text-bee-yellow">-40% затрат</span>
 </div>
 <div className="flex flex-col sm:flex-row sm:items-end justify-between border-b border-bee-border pb-4 gap-1 sm:gap-0">
 <span className="text-xs md:text-sm uppercase tracking-widest text-bee-white/40">Складские площади</span>
 <span className="text-lg md:text-xl font-bold text-bee-yellow">0 м² [Zero]</span>
 </div>
 <div className="flex flex-col sm:flex-row sm:items-end justify-between border-b border-bee-border pb-4 gap-1 sm:gap-0">
 <span className="text-xs md:text-sm uppercase tracking-widest text-bee-white/40">Минимальный заказ (MOQ)</span>
 <span className="text-lg md:text-xl font-bold text-bee-yellow">1 единица</span>
 </div>
 </div>
 </div>
 </div>
 ...
 <span className="block text-lg md:text-2xl font-black uppercase tracking-tight">{n.node}</span>
 ...
 <h3 className="text-xl md:text-2xl font-bold uppercase tracking-tight mb-4 group-hover:text-bee-yellow transition-colors break-words">
 {feature.title}
 </h3>
 ...
 {feature.id ==="01" ? <>Black Bee — это не один принтер в гараже, а промышленная сеть узлов по всей стране. <span className="whitespace-normal xl:whitespace-nowrap">Это гарантирует бесперебойность.</span></> : feature.description}
 </p>
 </div>
